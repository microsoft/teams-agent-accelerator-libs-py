...
async def test_embedding_calls_litellm_aembedding(mock_embedding):   
    ...
    lm = LLMService(embedding_model=embedding_model, api_base=api_base, api_version=api_version, api_key=api_key, **args)
...
async def test_embeddings_openai(config):
    ...
    assert len(res.data[0]['embedding']) >= 512 # 512 is the smallest configurable embedding size for the text-embedding-3-small model
